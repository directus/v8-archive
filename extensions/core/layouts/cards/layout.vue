    ...
    <div class="toolbar">
      ...
      <div class="sort-select">
        <select @input="setSort($event.target.value)" :value="sortedOn">
          <option v-for="(fieldInfo, name) in sortableFields" :key="name" :value="name">{{ $helpers.formatTitle(name) }}</option>
        </select>
...
export default {
  ...
  computed: {
    ...
    sortedOn() {
      ...
      if (sortableFieldNames && viewQuerySort && sortableFieldNames.some(sortableFieldName => sortableFieldName === viewQuerySort)) {
