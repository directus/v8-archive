<template>
  <div v-if="value" :class="[change < 0 ? 'down' : 'up', change == 0 ? '' : 'colorize', 'quote']">
    <div class="stock-container" :title="companyName">
      <span class="symbol">{{ displayValue }}:</span><span class="latest">{{latestPrice}}</span><span class="currency">USD</span>
    </div>
    <div class="change-container" @click.prevent="swapUnits()" :title="$t('interfaces-stock-swap_units')">
      <span v-if="showUnit" class="change units">{{change}}</span><span v-else class="change percent">{{changePercent}}</span>
    ...
    displayValue() {
      ...
      this.$api.axios
        ...
        .then(function(response) {
          ...
          vm.companyName = response.data.quote.companyName + " (" + response.data.quote.primaryExchange + ")";
