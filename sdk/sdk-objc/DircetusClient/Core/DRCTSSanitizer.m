...
NSString * const kDRCTSApplicationJSONType = @"application/json";

NSString * DRCTSPercentEscapedStringFromString(NSString *string) {
    static NSString * const kDRCTSCharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString * const kDRCTSCharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet * allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kDRCTSCharactersGeneralDelimitersToEncode stringByAppendingString:kDRCTSCharactersSubDelimitersToEncode]];
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[DRCTSQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[DRCTSObject class]]) {
...
- (NSString *) selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kDRCTSApplicationJSONType;
        }
